...
static socketBufferNode *newBufferPool(int32_t num)
{
  ...
  socketBufferNode *pool = (socketBufferNode *)util::memory::malloc(sizeof(struct socketBufferNode) * num);
  assert(pool);
  for (int i = 0; i < num; i++)
  {
    pool[i]._data = NULL;
...
public:
  ...
  std::string doReadAll()
  {
    ...
    {
      struct socketBufferNode *current = m_buffer._head;
      b.append(current->_data + m_buffer._offset, current->_sz - m_buffer._offset);
  ...
  bool isDisconnected()
  {
    return !(m_connected || !m_connecting.empty());
  }

  bool isConnecting()
  {
    return !m_connecting.empty();
  }

  boost::any getConnecting()
  {
    return m_connecting;
  }
  ...
  void callback(int32_t id, const char *addr)
  {
    m_func(id, addr);
  }
...
private:
  ...
  std::string popString(int sz, int skip)
  {
    ...
    for (;;)
    {
      ...
      if (realSz > 0)
      {
        b.append(current->_data + m_buffer._offset, (realSz < bytes) ? realSz : bytes);
      }
...
boost::any staticSocketUnPack(void *param, void *data,
                              uint32_t size)
{
...
void onAccept(uintptr_t opaque, int32_t id, int32_t clientId,
              char *addr)
{
...
void onData(uintptr_t opaque, int32_t id, uint32_t size,
            char *data)
{
  ptrSocket s = getSocket(id);
  if (s == nullptr)
...
void onWarning(int32_t id, int size)
{
  ...
  SYSLOG_ERROR(ptr->handle(), "WARNING: {} K bytes need to send out (fd = {})", size, id);
}

void onUdp(uintptr_t opaque, int32_t id, int size, char *data,
           char *addr) {}
...
void socketApi::doRequire(module::actorComponent *cpt)
{
  cpt->doRegisterProtocol({module::messageId::M_ID_SOCKET,
                           &staticSocketDispatch,
                           nullptr,
                           &staticSocketUnPack});
}

int32_t socketApi::doListen(std::shared_ptr<module::actor> ptr, const char *addr, int port, int backlog)
{
  return INST(network::socketSystem, doSocketListen, ptr->handle(), addr, port, backlog);
}

boost::any socketApi::doConnection(aSharedPtr ptr, const char *addr, int port)
{
  int32_t id = INST(network::socketSystem, doSocketConnect, ptr->handle(), addr, port);
