...
private:
  static void staticSocketDispatch(void *param, int32_t session, uint32_t src, boost::any data);
  static boost::any staticSocketUnPack(void *param, void *data, uint32_t size);

  static void onAccept(uintptr_t opaque, int32_t id, int32_t clientId, char *addr);
  ...
  static void onUdp(uintptr_t opaque, int32_t id, int size, char *data, char *addr);
  ...
  static uint32_t pushBuffer(wfSocketBuffer *ptrBuffer, char *data, uint32_t sz);
