...
int32_t actorComponent::doInit(actor *parent, void *parm)
{
  ...
  doRegisterProtocol({messageId::M_ID_ERROR, &actorComponent::staticErrorDispatch, nullptr, &actorComponent::staticErrorUnPack});
  return 0;
}
...
void actorComponent::doWait(struct coEntery co)
{
  ...
  removeSleepSusped(co._entry._id);
...
bool actorComponent::doWakeup(struct coEntery co)
{
  ...
  operation::worker::wakeup(co._entry, true);
  return true;
}

void actorComponent::quit()
{

  for (auto suspedIt = m_suspedSession.begin();
       suspedIt != m_suspedSession.end();
       suspedIt++)
  {
    INST(actorSystem, doSendMessage, m_parent->handle(), m_parent->handle(), messageId::M_ID_ERROR, suspedIt->first, (void *)"");
  }

  INST(actorSystem, doSendMessage, m_parent->handle(), m_parent->handle(), messageId::M_ID_QUIT, 0, (void *)"");
}
...
int32_t actorComponent::dispatchMessage(struct message *msg)
{
  ...
  if (msgId == messageId::M_ID_RESOUE)
  {
    ...
    else
    {
      ...
      removeSusped(msgSession);
      operation::worker::wakeup(tmpEntry, true);
      return 0;
    }
  }
...
void actorComponent::staticErrorDispatch(void *param, int32_t session, uint32_t src, boost::any data)
{
...
void actorComponent::errorDispatch(int32_t errorSession, uint32_t errorSrc, boost::any &data)
{
  ...
  if (ptrCo == nullptr)
  {
    ...
    {
      SYSLOG_ERROR(m_parent->handle(), "Source Address [:{:08x}] Error:{}", errorSrc, er._message);
      return;
    }
  }
  else
  {
    ...
    operation::worker::wakeup(tmpEnter, false);
  }
}

boost::any actorComponent::staticErrorUnPack(void *param, void *data, uint32_t size)
{
