...
int32_t worker::isBusy() {
  ...
  return clock::instance()->now() >
         m_ntsTick + coroutineOptions::instance()->_single_timeout_us;
}
...
bool worker::isExpire(struct suspendEntry const &entry) {
  ...
  if (entry._id != entry._tk->_supperId) {
    return true;
  }
  return false;
}
...
bool worker::wakeup(struct suspendEntry const &entry,
  ...
  auto wrk = entry._tk->_lpWorker;

  return wrk ? wrk->wakeupBySelf(entry._tk, entry._id, functor) : false;
}
