...
void scheduler::createTask(taskFunc const &fn, size_t const &stackSize) {
  task *t = new task(fn, stackSize);
...
void scheduler::dispatcherWork() {
  while (!m_shutdown) {
    std::this_thread::sleep_for(std::chrono::microseconds(
        coroutineOptions::instance()->_dispatcher_thread_interval_us));
