...
uint32_t actor::doInit(const char *name, void *parm)
{
  ...
  if (!ptrDLL)
  {
    SYSLOG_ERROR(0, "Library file not open {}", name);
    return 0;
  }
  ...
  if (!ptrCPT)
  {
    SYSLOG_ERROR(0, "{} component creation failed", name);
    return 0;
  }
  ...
  if (m_handle == 0)
  {
    SYSLOG_ERROR(0, "acotr resource is exhausted");
  ...
  {
    SYSLOG_INFO(m_handle, "LAUNCH {} {}", name, parm ? (const char *)parm : "");
    return m_handle;
  }
  else
  {
