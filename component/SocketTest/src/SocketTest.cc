...
WOLF_C_API void *SocketTest_create() {
  SocketTest *ptr = new SocketTest();
  assert(ptr);
  return (void *)ptr;
}

WOLF_C_API void SocketTest_signal(void *inst, int signal) {}

WOLF_C_API void SocketTest_release(void *inst) {
  SocketTest *ptr = (SocketTest *)inst;
  delete ptr;
}